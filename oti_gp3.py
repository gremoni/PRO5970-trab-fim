def oti_gp3():
    ...
    m.setObjective(
        gp.quicksum((spot[i]-dict_term['ct'][t])*GT[t,i] for t in term_gp3 for i in list_i)
        +gp.quicksum((spot[i]-dict_hid['ch'][h])*GH[h,i] for h in hid_gp3 for i in list_i)
        -beta*gp.quicksum(DFGH[h,i] for h in hid_gp3 for i in list_i),
        sense=gp.GRB.MAXIMIZE)
    
    
    # define as restrições
    #primeiro nível
    c12_1 = m.addConstrs(QOH[h,i] <= dict_hid['ghmax'][h] for h in hid_gp3 for i in list_i)
    c12_2 = m.addConstrs(QOH[h,i] >= 0 for h in hid_gp3 for i in list_i)
    c13_1 = m.addConstrs(QOT[t,i] <= dict_term['gtmax'][t] for t in term_gp3 for i in list_i)
    c13_2 = m.addConstrs(QOT[t,i] >= 0 for t in term_gp3 for i in list_i)
    c14 = m.addConstrs(ear[h,i] == 
                       ear[h,i-1]-((vt[h,i-1]+ve[h,i]-dict_hid['inc'][h]))/1e6 
                       for h in hid_gp3 for i in list_i[1:])
    c15 = m.addConstrs(GH[h,i] - dict_hid['rho'][h]*vt[h,i] <= DFGH[h,i]  for h in hid_gp3 for i in list_i)
    c16 = m.addConstrs(-(GH[h,i] - dict_hid['rho'][h]*vt[h,i]) <= DFGH[h,i]  for h in hid_gp3 for i in list_i)
    c17 = m.addConstrs(DFGH[h,i] >= 0 for h in hid_gp3 for i in list_i)
    c18_1 = m.addConstrs(ear[h,i] <= dict_hid['vmax'][h] for h in hid_gp3 for i in list_i)
    c18_2 = m.addConstrs(ear[h,i] >= dict_hid['vmin'][h] for h in hid_gp3 for i in list_i)
    c18_3 = m.addConstrs(ear[h,T] >=  dict_hid['vobj'][h] for h in hid_gp3)
    ...
    c_teste12 = m.addConstrs(QOT[t,i] == QOT_gp2_ant[t,str(i)] for t in term_gp2 for i in list_i)
    ...
    for t in term_gp3:
        for i in range(1,T+1):
            COFT_gp3_ant["{}".format(t), "{}".format(i)] = COFT.select(t, i)[0].X
    ...
    for t in term_gp3:
        for i in range(1,T+1):
            QOT_gp3_ant["{}".format(t), "{}".format(i)] = QOT.select(t, i)[0].X
    ...
    for h in hid_gp3:
        for i in range(1,T+1):
            COFH_gp3_ant["{}".format(h),  '{}'.format(i)] = COFH.select(h, i)[0].X
    ...
    for h in hid_gp3:
        for i in range(1,T+1):
            QOH_gp3_ant["{}".format(h), "{}".format(i)] = QOH.select(h, i)[0].X 
