def oti_gp2():
    ...
    m.setObjective(
        gp.quicksum((spot[i]-dict_term['ct'][t])*GT[t,i] for t in term_gp2 for i in list_i)
        +gp.quicksum((spot[i]-dict_hid['ch'][h])*GH[h,i] for h in hid_gp2 for i in list_i)
        -beta*gp.quicksum(DFGH[h,i] for h in hid_gp2 for i in list_i),
    ...
    #primeiro nível
    c12_1 = m.addConstrs(QOH[h,i] <= dict_hid['ghmax'][h] for h in hid_gp2 for i in list_i)
    c12_2 = m.addConstrs(QOH[h,i] >= 0 for h in hid_gp2 for i in list_i)
    c13_1 = m.addConstrs(QOT[t,i] <= dict_term['gtmax'][t] for t in term_gp2 for i in list_i)
    c13_2 = m.addConstrs(QOT[t,i] >= 0 for t in term_gp2 for i in list_i)
    c14 = m.addConstrs(ear[h,i] == 
                       ear[h,i-1]-((vt[h,i-1]+ve[h,i]-dict_hid['inc'][h]))/1e6 
                       for h in hid_gp2 for i in list_i[1:])
    c15 = m.addConstrs(GH[h,i] - dict_hid['rho'][h]*vt[h,i] <= DFGH[h,i]  for h in hid_gp2 for i in list_i)
    c16 = m.addConstrs(-(GH[h,i] - dict_hid['rho'][h]*vt[h,i]) <= DFGH[h,i]  for h in hid_gp2 for i in list_i)
    c17 = m.addConstrs(DFGH[h,i] >= 0 for h in hid_gp2 for i in list_i)
    c18_1 = m.addConstrs(ear[h,i] <= dict_hid['vmax'][h] for h in hid_gp2 for i in list_i)
    c18_2 = m.addConstrs(ear[h,i] >= dict_hid['vmin'][h] for h in hid_gp2 for i in list_i)
    c18_3 = m.addConstrs(ear[h,T] >=  dict_hid['vobj'][h] for h in hid_gp2)

    #conhecimento das ofertas dos demais players
    c_teste1 = m.addConstrs(QOT[t,i] == QOT_gp1_ant[t,str(i)] for t in term_gp1 for i in list_i)
    c_teste12 = m.addConstrs(QOT[t,i] == QOT_gp3_ant[t,str(i)] for t in term_gp3 for i in list_i)
    c_teste2 = m.addConstrs(QOH[h,i] == QOH_gp1_ant[h,str(i)] for h in hid_gp1 for i in list_i)
    c_teste21 = m.addConstrs(QOH[h,i] == QOH_gp3_ant[h,str(i)] for h in hid_gp3 for i in list_i)
    c_teste3 = m.addConstrs(COFT[t,i] == COFT_gp1_ant[t,str(i)] for t in term_gp1 for i in list_i)
    c_teste31 = m.addConstrs(COFT[t,i] == COFT_gp3_ant[t,str(i)] for t in term_gp3 for i in list_i)
    c_teste4 = m.addConstrs(COFH[h,i] == COFH_gp1_ant[h,str(i)] for h in hid_gp1 for i in list_i)
    c_teste41 = m.addConstrs(COFH[h,i] == COFH_gp3_ant[h,str(i)] for h in hid_gp3 for i in list_i)
    
    #segundo nível
    c23 = m.addConstrs(gp.quicksum(GT[t,i] for t in list_term) 
                       + gp.quicksum(GH[h,i] for h in list_hid) 
                       == sum(dict_cons['consumo'].values()) for i in list_i)
    ...
    for t in term_gp2:
        for i in range(1,T+1):
            COFT_gp2_ant["{}".format(t), "{}".format(i)] = COFT.select(t, i)[0].X
    ...
    for t in term_gp2:
        for i in range(1,T+1):
            QOT_gp2_ant["{}".format(t), "{}".format(i)] = QOT.select(t, i)[0].X
    ...
    for h in hid_gp2:
        for i in range(1,T+1):
            COFH_gp2_ant["{}".format(h),  '{}'.format(i)] = COFH.select(h, i)[0].X
    ...
    for h in hid_gp2:
        for i in range(1,T+1):
            QOH_gp2_ant["{}".format(h), "{}".format(i)] = QOH.select(h, i)[0].X 
